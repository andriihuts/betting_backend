...
@section('content')
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
        ...
        <nav aria-label="breadcrumb">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">IRC</li>
            </ol>
            <h6 class="font-weight-bolder mb-0">IRC</h6>
        </nav>
          ...
          <div class="card">
            ...
            <div class="card-body p-3 irc_form_box">              
              <form action="#">
                ...
                <button class="btn bg-gradient-primary w-100 mb-0" id="set_amount_irc">Set Amount for IRC</button>
              </form>
            </div>
          </div>
        </div>        
...
<script>
    $(document).ready(function () {        
        $(document).on('click', '#set_amount_irc', function (e) {
            e.preventDefault();            
            const amount = $('input[type="number"]').val();
            const moneyType = $('input[name="currency_type"]:checked').attr('id'); // Selected currency type
            ...
            // Determine the currency type ID based on the radio button ID
            ...
            updateIRCAmount(amount, moneyTypeValue);
        });
    });

    // Update IRC amount and currency type in backend  
    function updateIRCAmount(amount, moneyType) {        
        const csrfToken = '{{ csrf_token() }}';
                
        const rateForm = $(document).find('.irc_form_box');
